...
function previewImage(fileInputId, previewId) {
    ...
    // Reset textarea & output jika user ganti gambar
    if (fileInputId === 'embedFile') {
        document.getElementById('message').value = '';
        document.getElementById('embedOutput').innerHTML = '';
        document.getElementById('clearEmbedBtn').style.display = 'inline-block';
    } else if (fileInputId === 'extractFile') {
        document.getElementById('extractOutput').innerHTML = '';
        document.getElementById('clearExtractBtn').style.display = 'inline-block';
    }
...
function clearEmbed() {
    ...
    document.getElementById('embedOutput').innerHTML = '';
    document.getElementById('clearEmbedBtn').style.display = 'none';
}
...
function clearExtract() {
    ...
    document.getElementById('extractOutput').innerHTML = '';
    document.getElementById('clearExtractBtn').style.display = 'none';
}
...
async function embedMessage() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            previewImg.src = result.file;
            previewImg.alt = 'Gambar Stego';
            previewImg.style.maxWidth = '100%';
            previewImg.style.marginTop = '1rem';
        
            output.innerHTML = `<p>${result.message}</p>`;
            output.appendChild(previewImg);
            output.innerHTML += `<br><a href="${result.file}" download>⬇️ Unduh Gambar Stego</a>`;
        }
        else {
